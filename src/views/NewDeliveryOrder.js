...
const NewDeliveryOrder = () => {
  const [formData, setFormData] = useState({
    pickupLocation: '', // Added this field
    deliveryAddress: '',
    description: '',
    weight: '',
    speed: 'standard',
  ...
  const handleChange = (e) => {
    ...
    if (name === 'weight' || name === 'speed' || name === 'distance') {
      ...
      const distance = parseFloat(formData.distance) || 10; // Provide a default distance if not set
  ...
  return (
    <div className="new-delivery-order-container">
      <div className="new-delivery-order-navbar">
        <Navbar1 rootClassName="navbar-root-class-name" />
